...
## 0.9.11 — 2026-09-15（CF-Server-Monitor 移植版）

**修复（用户报告）**：PC 打开服务器详情后，右侧详情面板「一直被拉上去」——面板内的滚动位置被反复拉回顶部。

**根因两条，互相放大**：

1. **详情/抽屉打开时，PC 端每个实时批次都立刻整壳重建。** `schedulePatchOrRender()` 遇到叠加层就转交
   `scheduleStatusRender()`，而那里的合并节流只对移动布局生效
   （`if (!mobile || manual) { renderApp(); return; }`）→ 桌面无节流：在真实站点节奏（约 0.1 秒/批）下
   实测 **30+ 次整壳重建/秒**，每次都把抽屉 DOM 连同图表销毁重建。
2. **滚动位置写回晚一帧。** `restoreRenderContinuity()` 整段跑在 `requestAnimationFrame` 里：换壳后新容器
   停在 0，要等下一帧才写回。一旦帧率落后于重建节奏（30+ 次/秒必然落后），下一次
   `captureRenderContinuity()` 就会读到这个中间态 0 并原样写回 → 位置被**永久钉在顶部**。

**复现与证据**：把 mock 调到 0.1 秒/批（真实站点节奏）、并在验证环境里把 rAF 延迟设为 500ms（模拟被拖长的
一帧）后，抽屉滚动 **24/24 采样全部为 0**；同一窗口内整壳重建 183 次。

**修法**：

- 滚动写回改为**同步**：新增 `restoreScrollContinuity()`，在 `app.innerHTML` 赋值的**同一任务**内写回，并按
  新内容尺寸夹紧（`status-ribbon` / `filter-group` / `drawer-scroll`）；`restoreRenderContinuity()` 只保留
  「焦点与选区」放下一帧（与滚动无关）。
- 叠加层打开时 PC 与移动共用合并窗口：新增 `OVERLAY_STATUS_RENDER_IDLE_MS = 180`，桌面在叠加层状态下
  改为窗口内只渲染一次（手动刷新仍立即渲染）。

**实测（同一条件：0.1 秒/批 + 500ms 帧延迟）**：

| 指标 | 修复前 | 修复后 |
|---|---|---|
| 抽屉滚动采样为 0 的比例 | 24/24 = **100%**（钉在顶部） | **0/24 = 0%** |
| 稳态整壳重建 | 30+ 次/秒（窗口内 183 次） | **4.8 次/秒** |
| 桌面无叠加层（列表） | 补丁路径 | 不变：0 次重建 / 28 次补丁 |
| 移动端稳态 | 补丁路径 | 不变：200 次补丁 / 0 次重建，数值与时钟正常更新 |

**遗留**：叠加层打开时仍是整壳重建（只是频率降到约 5–12 次/秒），面板内容仍会被重建 —— 彻底做法是让
`patchLiveValues()` 支持抽屉/详情字段（补丁而非重建），本次未做。
