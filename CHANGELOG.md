...
- **修复**：移动布局同样走数值补丁路径（卡片 DOM 与桌面一致，差异只在 CSS；其余弹层/抽屉状态仍退回
  整页渲染）；旗帜改为 `loading="eager" decoding="sync"`，并对每个地区码预热解码位图且**持有引用**
  （引用在则解码结果不会被回收），覆盖仍会发生整页渲染的场景（10 秒兜底对账、切换视图等）。
...
- **「无空白帧」的判据与证据**：稳态不再重建节点只能证明「没有重建」，**不等于**绘制时无空白帧。
  ...
  `complete=true, naturalWidth=16`**，故那两次重建也不会画空白帧。**限定**：该结论来自本机 WebKit
  实测，`decoding`/持有引用本身只提供缓解（浏览器不承诺「有 JS 引用就不淘汰解码结果」），
  真机连续目视仍是唯一终局验收。
